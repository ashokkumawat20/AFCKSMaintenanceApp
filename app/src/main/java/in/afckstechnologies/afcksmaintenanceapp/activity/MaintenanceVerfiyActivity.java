...
import in.afckstechnologies.afcksmaintenanceapp.utils.SmsReceiver;
import in.afckstechnologies.afcksmaintenanceapp.utils.WebClient;

public class MaintenanceVerfiyActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_maintenance_verfiy);
    ...
    private class smspassAvailable extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute() {
            ...
            mProgressDialog = new ProgressDialog(MaintenanceVerfiyActivity.this);
    ...
    private class userAvailable extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void args) {
            if (status) {
                ...
                String message = "AFCKS Maintenance code " + id;
...
// Process line...
    ...
    private class submitData extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            jsonObj = new JSONObject() {
                {
                    try {
                        put("id", user_id);
                        put("maintenance_mobile_deviceid", deviceId);
            ...
            Log.i("json", "json" + jsonObj);
            trainerResponse = serviceAccess.SendHttpPost(Config.URL_UPDATEMAINTENANCEDETAILS, jsonObj);
            Log.i("resp", "centerListResponse" + trainerResponse);
        ...
        protected void onPostExecute(Void args) {
            if (status) {
                mProgressDialog.dismiss();
                try {
                    prefEditor.putString("maintenance_user_id", user_id);
                    prefEditor.putString("phone_no", phoneno);
                    prefEditor.putString("maintenance_mobile_deviceid", deviceId);
                    prefEditor.putString("maintenance_user_name", jsonObject.getString("trainer_name"));
                    prefEditor.commit();
                } catch (JSONException e) {
                    e.printStackTrace();
                }
                ...
                Toast.makeText(getApplicationContext(), msg, Toast.LENGTH_LONG).show();
                finish();
                Intent intent = new Intent(MaintenanceVerfiyActivity.this, FixedAssetsActivity.class);
